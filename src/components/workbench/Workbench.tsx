...
import { executeQuery } from "@/lib/resumedb/executeQuery";
import {
...
import type { HistoryItem, QueryMode } from "@/lib/resumedb/types";
...
export function Workbench() {
  ...
  const appendHistory = useCallback((queryId: string | null, displayName: string, durationMs: number, status: "ok" | "error") => {
    setHistory((previous) => [
      {
  ...
  const handleRunQueryFromCatalog = useCallback(
    (queryId: string) => {
      const query = getQueryDefinitionById(queryId);
      if (!query) {
        return;
      }
      ...
      setTabs((previous) => [...previous, newTab]);
      setActiveTabId(newTab.id);
      appendHistory(result.queryId, query.simpleLabel, durationMs, result.status);
    },
    [appendHistory, mode, tabs.length],
  );
    ...
    <main className="workbench-root">
      ...
      <div className="workbench-grid">
        ...
        <QuickActions
          ...
          history={history}
          onRunQuery={handleRunQueryFromCatalog}
